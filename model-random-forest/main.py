...
app = FastAPI(title="Random Forest Phishing Detector")
...
async def load_model():
    ...
    try:
        ...
        model_data = PickleCompatLoader.load("random_forest_model.pkl")
...
async def health_check():
    return {"status": "healthy", "model": "Random Forest", "loaded": model is not None}
...
async def predict(email: EmailRequest):
    ...
    try:
        ...
        # For this model, scaler was trained on manual features only, not combined
        # So we do NOT scale the combined vector
...
@app.get("/")
async def root():
    return {"service": "Random Forest Phishing Detector", "status": "running"}
