#!/usr/bin/env bash
# Email Security Dashboard - Clean Start Script
# Full rebuild with fresh volumes (deletes all data)
# For normal start, use: ./start.sh
...
stop_and_clean() {
    print_msg "$BLUE" "🔄 Stopping existing containers and removing volumes..."

    if docker compose ps --quiet 2>/dev/null | grep -q .; then
        print_msg "$YELLOW" "⏸️  Stopping containers and wiping all data..."
        docker compose down -v 2>&1 | grep -v "the attribute .version. is obsolete" || true
        print_msg "$GREEN" "✅ Stopped containers and removed volumes"
    else
        # Even if no containers running, remove volumes to ensure clean state
        print_msg "$YELLOW" "🗑️  Removing any existing volumes..."
        docker compose down -v 2>/dev/null || true
        print_msg "$GREEN" "✅ Volumes cleaned"
    fi
}
...
wait_for_services() {
    ...
    elapsed=0
    while [ $elapsed -lt $max_wait ]; do
        if docker compose ps mailpit 2>/dev/null | grep -q "healthy"; then
...
show_status() {
    ...
    print_msg "$YELLOW" "   2. Wait for email-seeder to populate MailPit (~1-2 minutes)"
    print_msg "$YELLOW" "   3. Email-fetcher will automatically load emails into the system"
    print_msg "$YELLOW" "   4. Click 'Process All Unprocessed' to run phishing detection"
    echo ""
...
main() {
    print_msg "$BLUE" "=========================================="
    print_msg "$BLUE" "🧹 Email Security Dashboard - Clean Start"
    print_msg "$BLUE" "=========================================="
    echo ""

    print_msg "$YELLOW" "⚠️  This will:"
    print_msg "$YELLOW" "   - Stop all containers"
    print_msg "$YELLOW" "   - Remove ALL volumes (database, emails, etc.)"
    print_msg "$YELLOW" "   - Rebuild images from scratch (no cache)"
    print_msg "$YELLOW" "   - Start with completely fresh state"
    echo ""

    read -p "Continue with clean start? (y/N): " -n 1 -r
    echo
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
        print_msg "$YELLOW" "Cancelled. Use ./start.sh for normal start."
        exit 0
    fi
